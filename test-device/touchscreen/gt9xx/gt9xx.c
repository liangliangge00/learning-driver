...
static int lcd = -1;
module_param(lcd, int, 0400);
MODULE_PARM_DESC(lcd, "get lcd module from user...\n");

static int debug= 0;	// 0:disable(default) 1:enable
module_param(debug, int, 0600);
MODULE_PARM_DESC(debug_int, "debug enable for printk touch point\n");
...
#ifdef GTP_CONFIG_OF
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 cfg_index);
#endif
...
//#if	(!GTP_CONFIG_OF)
    ...
    cfg_index = gtp_config->gtp_cfg_select;
	ret = gtp_parse_dt_cfg(&ts->client->dev, &config[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, cfg_index);
	if (ret < 0) {
    ...
    GTP_INFO("Config group%d used, length: %d", cfg_index, ts->gtp_cfg_len);
...
static void gtp_parse_dt(struct device *dev)
{
    ...
    of_property_read_u32(np, "goodix_rst_gpio", &gtp_rst_gpio);
    of_property_read_u32(np, "goodix_irq_gpio", &gtp_int_gpio);
	...
	of_property_read_u32(np, "gtp_cfg_select", &(gtp_config->gtp_cfg_select));

	/* selcect resolution and init config according to lcd */	
#if 0
	if(strcmp(lcd, "mipi-7-1024x600") == 0){
		gtp_config->gtp_resolution_x = 1024;
		gtp_config->gtp_resolution_y = 600;
		gtp_config->gtp_cfg_select   = 0;
	}
	else if(strcmp(lcd, "mipi-8-800x1280") == 0){
		gtp_config->gtp_resolution_x = 800;
		gtp_config->gtp_resolution_y = 1280;
		gtp_config->gtp_cfg_select   = 1;
	}
	else if(strcmp(lcd, "mipi-10-800x1280") == 0){
		gtp_config->gtp_resolution_x = 800;
		gtp_config->gtp_resolution_y = 1280;
		gtp_config->gtp_cfg_select   = 2;
	}
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 cfg_index)
{
	...
	sprintf(cfg_name, "goodix,cfg-group%d", cfg_index);	
