...
static Judge_Rule Test_Rule={
	...
	//dac
	{
		{{85,85,80,75},	  	//up limit
			{10,10,10,10}}  //low limit	
		,
		{{4,3,2,1},	  		//up	 limit
			{0,0,0,0}}      //low limit	
	},
...
static void ReadFrame(void)
{
	if(cpu_type == CPU_TYPE_1682 || (cpu_type==CPU_TYPE_1688 && core_vers < CORE_01040001))//
	{
		if(base_addr == 0){
			...
			if(read_type == GATHER_DATA_BASE)
				ReadDataShort(0x5f80/*0x59d0*/ + ori_frame*23*12*2);
			else if(read_type == GATHER_DATA_REFE)
				ReadDataShort(0x5f80/*0x59d0*/+23*12*2*2);
			else if(read_type == GATHER_DATA_SUB)
				ReadDataInt(0x5f80/*0x59d0*/+23*12*2*4,14,TRUE);
		}
	}
...
//	memset(test_result,'\0',sizeof(test_result));
	memset(up_origin,'\0',1024);
	memset(low_origin,'\0',1024);
...
int gsl_obtain_array_data_ogv(unsigned int *gsl_ogv_new,int i_max,int j_max)
{
	int i,j;
	unsigned int ret = 1;
	//printk("[%s]:enter rawdata value function!\n",__func__);

	int j_tmp = (j_max > 24 ? 24 : j_max);
	int i_tmp = (i_max > 32 ? 32 : i_max);

	printk("[%s]:j_tmp = %d, i_tmp = %d\n",__func__,j_tmp,i_tmp);
	
	for(i=0;i<i_tmp;i++){
		for(j=0;j<j_tmp;j++){
			gsl_ogv_new[i*11+j]=(int)gsl_ogv[i][j];
			printk("==%04d",gsl_ogv_new[i*11+j]);
		}
		//printk("\n");
	}

	return ret;
	//printk("[%s]:obtain rawdata end!\n",__func__);

}
#else
int gsl_obtain_array_data_ogv(short *ogv,int i_max,int j_max)
{
	int i;
	unsigned int ret = 1;
	int j_tmp = (j_max > 24 ? 24 : j_max);
